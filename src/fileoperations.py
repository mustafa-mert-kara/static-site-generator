...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path):
    for val in os.listdir(dir_path_content):
        if ".md" in val:
            generate_page(os.path.join(dir_path_content,val),template_path,os.path.join(dest_dir_path,"index.html"))
        elif os.path.isdir(os.path.join(dir_path_content,val)):
            os.mkdir(os.path.join(dest_dir_path,val))
            generate_pages_recursive(os.path.join(dir_path_content,val),template_path,os.path.join(dest_dir_path,val))